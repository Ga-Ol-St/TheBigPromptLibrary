# opencode - `muse-spark-1.2` (responses-dialect template) (unverified)

- **Contributed by**: [xizhuomengcontin](https://github.com/xizhuomengcontin)
- **Contributed on**: 09/02/2026
- **Source**: opencode `run` mode, model `muse-spark-1.2-contributor-free`, Responses API dialect
- **Public copy**: [prompt/OPENCODE/muse-spark-1-2-contributor-free-system-prompt.md](https://github.com/Continuum-AI-Corp/OrcaReplay/blob/main/prompt/OPENCODE/muse-spark-1-2-contributor-free-system-prompt.md)
- **Note**: Captured on the wire rather than extracted from the shipped bundle. A local proxy in front of the model API recorded the request the harness actually sent, so this is the prompt as the harness assembled it at runtime on one machine, including the blocks that only exist at runtime. Reproducible in one command with [OrcaReplay](https://github.com/Continuum-AI-Corp/OrcaReplay): `node capture/capture.mjs opencode --model muse-spark-1.2-contributor-free`. Machine-identifying strings - home directory, username, git name and email, gateway host, OS build, uuids, hex runs of 32 or more - are replaced with `{PLACEHOLDER}` tokens by a scrubber that aborts the capture rather than write a file if anything identifying survives the pass; nothing else is reworded, reordered or removed. The second of the three templates. It opens "You are OpenCode, a coding agent that helps users with software engineering tasks" - capital C, different first line - and is the only one of the seven models measured that opencode drives over the responses dialect rather than chat completions. 11 tool definitions. The prompt measured 10,249 characters on the wire; the file below is a few hundred shorter because the placeholders are shorter than the paths they replace.
