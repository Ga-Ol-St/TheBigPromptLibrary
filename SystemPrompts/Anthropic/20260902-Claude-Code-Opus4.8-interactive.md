# Claude Code - Opus 4.8, interactive (unverified)

- **Contributed by**: [xizhuomengcontin](https://github.com/xizhuomengcontin)
- **Contributed on**: 09/02/2026
- **Source**: Claude Code CLI v2.1.258.18d (`cc_entrypoint=cli`), interactive TTY session on Windows, model `claude-opus-4-8`
- **Public copy**: [prompt/CLAUDECODE/claude-opus-4-8-system-prompt.md](https://github.com/Continuum-AI-Corp/OrcaReplay/blob/main/prompt/CLAUDECODE/claude-opus-4-8-system-prompt.md)
- **Note**: Captured on the wire rather than extracted from the shipped bundle. A local proxy in front of the model API recorded the request the harness actually sent, so this is the prompt as the harness assembled it at runtime on one machine, including the blocks that only exist at runtime. Reproducible in one command with [OrcaReplay](https://github.com/Continuum-AI-Corp/OrcaReplay): `node capture/capture.mjs claude --model claude-opus-4-8`. Machine-identifying strings - home directory, username, git name and email, gateway host, OS build, uuids, hex runs of 32 or more - are replaced with `{PLACEHOLDER}` tokens by a scrubber that aborts the capture rather than write a file if anything identifying survives the pass; nothing else is reworded, reordered or removed. Interactive mode. 33 tool definitions on this model against 35 on Opus 5 and Fable 5.1 through the same harness build. The prompt measured 19,083 characters on the wire; the file below is a few hundred shorter because the placeholders are shorter than the paths they replace.
